...
def find_tables(
    ...
    # === Minimal, deterministic column stabilization =======================
    # If caller没有显式传入竖线，且当前策略仍是 "lines"：
    # 对抽出的竖线X坐标做 2pt 聚类，若聚类后主竖线数≥6，则仅把竖线改为 explicit，
    # 横线仍然使用默认 "lines"（不补线、不额外启发式）。
    if vertical_lines is None and vertical_strategy == "lines":
        # 收集本页所有竖线的 x0
        v_edges = [e for e in EDGES if e.get("orientation") == "v"]
        xs = sorted({round(e["x0"], 2) for e in v_edges})
        # 2pt 内合并（最小改动：简单线性聚类）
        merged = []
        for x in xs:
            if not merged or abs(x - merged[-1]) > 2.0:
                merged.append(x)
            else:
                # 与上一值平均，进一步稳态
                merged[-1] = (merged[-1] + x) / 2.0
        # 去掉页边 12pt 以内的线，避免装饰线干扰
        page_w = page.rect.width
        merged = [x for x in merged if 12.0 <= x <= page_w - 12.0]
        # 若主竖线数达到常见多列表格阈值（≥6 根 → 5 列）
        if len(merged) >= 6:
            settings["vertical_strategy"] = "explicit"
            settings["explicit_vertical_lines"] = merged
            # 与横线策略无关：保持使用调用方传入的 horizontal_strategy / horizontal_lines
            tset = TableSettings.resolve(settings=settings)
            page.table_settings = tset
    # ======================================================================

    # re-establish old setting
    TOOLS.set_small_glyph_heights(old_small)
...
if __name__ == "__main__":
    ...
    def main():
        ...
        for j, table in enumerate(tables.tables, start=1):
            print(f"表格 {j}: {table.row_count} 行 × {table.col_count} 列")
            try:
                data = table.extract()
                print("前几行示例：")
                for row in data[:5]:
